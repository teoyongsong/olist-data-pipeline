{{
    config(
        materialized='table',
        schema='dw'
    )
}}
with oi as (
    select * from {{ ref('stg_order_items') }}
),

orders as (
    select * from {{ ref('stg_orders') }}
),

customers as (
    select * from {{ ref('dim_customer') }}
),

products as (
    select * from {{ ref('dim_product') }}
),

sellers as (
    select * from {{ ref('dim_seller') }}
),

joined as (
    select
        oi.order_id,
        oi.order_item_id,
        c.customer_key,
        p.product_key,
        s.seller_key,
        orders.order_ts::date as order_date_key,
        orders.delivered_ts::date as delivery_date_key,
        oi.price,
        oi.freight_value,
        (oi.price + oi.freight_value) as total_item_value,
        case
            when orders.delivered_ts is not null
                 and orders.estimated_delivery_ts is not null
                 and orders.delivered_ts::date > orders.estimated_delivery_ts::date
            then true
            else false
        end as is_late_delivery,
        case
            when orders.order_ts::date = c.first_order_date then true
            else false
        end as is_first_order
    from oi
    join orders on orders.order_id = oi.order_id
    join customers c on c.customer_id = orders.customer_id
    left join products p on p.product_id = oi.product_id
    left join sellers s on s.seller_id = oi.seller_id
)

select
    row_number() over (order by order_id, order_item_id) as order_item_key,
    *
from joined
